...
theads.forEach((thead, index)=>{
    thead.addEventListener("click", ()=>{
        const sorted = tbodyChildren.sort((a, b)=>{
            const valueA = a.querySelector(`*:nth-child(${index + 1})`).textContent.split(" ").join("").replaceAll("$", "");
            const valueB = b.querySelector(`*:nth-child(${index + 1})`).textContent.split(" ").join("").replaceAll("$", "");
